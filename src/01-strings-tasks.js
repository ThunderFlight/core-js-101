...
function getRectangleString(a, b) {
  const gen = `│${' '.repeat(a - 2)}│\n`;
  const sq = `┌${'─'.repeat(a - 2)}┐\n${gen.repeat(b - 2)}└${'─'.repeat(a - 2)}┘\n`;
  return sq;
}
...
function encodeToRot13(a) {
  ...
  const arrK = a.split('');
  let res = '';
  for (let i = 0; i < arrK.length; i += 1) {
    for (let j = 0; j < arr.length; j += 1) {
      if (arrK[i] === symbols[j]) {
        res += `${symbols[j]}`;
      }
      if (arrK[i] === arrUpper[j]) {
        if (arrUpper[j] === 'N') {
          res += 'A';
        }
        if (j + 13 > arrUpper.length) {
          const g = j + 13 - arrUpper.length;
          res += `${arrUpper[g]}`;
        } else if (j + 13 < arrUpper.length) {
          if (j + 13 !== arrUpper.length) {
            res += `${arrUpper[j + 13]}`;
          } else {
            res += `${arrUpper[j]}`;
      ...
      if (arrK[i] === arr[j]) {
        ...
        if (j + 13 > arr.length) {
          const c = j + 13 - arr.length;
          res += `${arr[c]}`;
        } else if (j + 13 < arr.length) {
          if (j + 13 !== arr.length) {
            res += `${arr[j + 13]}`;
          } else {
            res += `${arr[j]}`;
